...
img_path = "C:/Users/LG/OCR/receipt/re1.jpg"
...
img_path2 = "C:/Users/LG/OCR/receipt/re2.jpg"
receipt2 = cv2.imread(img_path2, cv2.IMREAD_COLOR)
plt.figure(figsize=[5,5])
plt.imshow(receipt)

receipt2_gray = cv2.imread(img_path2,cv2.IMREAD_GRAYSCALE)
plt.figure(figsize=[5,5])
plt.imshow(receipt_gray)
...
receipt_bin = img_to_binary(receipt2_gray)
plt.figure(figsize=[5,5])
plt.imshow(receipt_bin)

img_noise2 = cv2.medianBlur(receipt2,ksize = 7)
plt.subplot(1,2,1)
plt.imshow(receipt2)
plt.xlabel("original")
plt.subplot(1,2,2)
...
closing = cv2.morphologyEx(receipt2, cv2.MORPH_CLOSE,kernel)
opening = cv2.morphologyEx(receipt2, cv2.MORPH_OPEN, kernel)
erosion = cv2.erode(receipt2,kernel,iterations = 1)
dilation = cv2.dilate(receipt2,kernel,iterations = 1)
...
#%%OCR 
##preprocessing result : the highest accuracy to detection text is origin 
